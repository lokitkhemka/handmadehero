...
internal void
CatStrings(size_t SourceACount, char *SourceA, size_t SourceBCount, char *SourceB,
...
//                         win32_sound_output *SoundOutput, real32 C, int PadX,
//                         int Top, int Bottom, DWORD Value, uint32 Color)
// {
...
#if 0
            //Testing the PlayCursor/WriteCursor update Frequency
            while(Running)
            {
                ...
                while(Running)
                {
                    ...
                    *NewKeyboardController = {};
                    ...
                    if(!GlobalPause)
                    {
                        ...
                        {
                            /*Sound Output Computation works as follows. When the PlayCursor
                            and WriteCursor are determined, the PlayCursor at the next
                            frame flip is predicted. Two cases are possible.

                            One, if the WriteCursor is before the Forecasted PlayCursor.
                            (Low Latency case).
                            In this case, the buffer is written by from WriteCursor
                            plus one frame and then to the next frame boundary.

                            If the WriteCursor is after the Forecasted PlayCursor,
                            i.e., in case of sound cards with high latency,
                            so the one frame audio is written plus some safety margin.
                            */
    #if HANDMADE_INTERNAL
                            ...
                            bool32 AudioCardHasLowLatency = (SafeWriteCursor < ExpectedFrameBoundaryByte);

                            DWORD TargetCursor = 0;
                            if(AudioCardHasLowLatency)
                            {
...
#if HANDMADE_INTERNAL
{
                        DWORD DebugPlayCursor, DebugWriteCursor;
